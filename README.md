...
## Setup
1. Create a new Tmux Session or attach to your existing session:
   ```bash
   tmux new-session -t <session-name>
   ```
3. Run the Fiftyone Podman/Docker using the command:
   ```bash
...
#### 4. `uniqueness.py`
Calculates the uniqueness of a given image in relation to the other images in the dataset.
##### Arguments:
- `--dataset_dir` (str, required): Path to the dataset directory.
- `--name` (str, required): Name of the dataset.
- `--dataset_type` (str, optional, default=`ImageDirectory`): Type of dataset.
...
**Example:**
```bash
python3 uniqueness.py --dataset_dir <path> --name <dataset_name> --dataset_type ImageDirectoryTree
```

#### 5. `representativeness.py`
Measures how well the dataset covers a variety of images, ensuring it isn't too repetitive.
##### Arguments:
- `--dataset_dir` (str, required): Path to the dataset directory.
- `--name` (str, required): Name of the dataset.
- `--dataset_type` (str, optional, default=`ImageDirectory`): Type of dataset.
...
**Example:**
```bash
python3 representativeness.py --dataset_dir <path> --name <dataset_name> --dataset_type ImageDirectoryTree
